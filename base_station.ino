...
#define HUE_BINS 8
struct MoodSample { uint8_t mood[3]; uint8_t hue[HUE_BINS]; };
...
void sampleMoodHist() {
  ...
  if (xSemaphoreTake(tblMutex, pdMS_TO_TICKS(20)) == pdTRUE) {
    for (uint16_t i = 0; i < badgeCount; i++) {
      ...
      uint8_t m = badges[i].lastMood; if (m < 3 && s.mood[m] < 255) s.mood[m]++;
...
void handleTimeline() {
  ...
  for (uint8_t i = 0; i < moodHistN; i++) {
    if (i) j += ",";
    j += "[" + String(moodHist[i].mood[0]) + "," + String(moodHist[i].mood[1]) + "," + String(moodHist[i].mood[2]) + "]";
...
<small style="margin-top:3px">chill &middot; social &middot; party</small>
...
const MOODCOL=['#3aa0ff','#3ad17a','#ff5e9c'];
